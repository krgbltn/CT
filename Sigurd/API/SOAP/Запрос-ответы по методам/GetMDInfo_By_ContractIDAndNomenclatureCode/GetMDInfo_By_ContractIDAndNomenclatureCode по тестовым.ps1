...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
    ...
    <soap:Body>
        <GetMDInfo_By_ContractIDAndNomenclatureCode xmlns="http://tempuri.org/">
            <ContractStrGUID>$contractStrGUID</ContractStrGUID>
            <NomenclatureCode>$nomenclatureCode</NomenclatureCode>
...
function Send-SOAPRequest {
    ...
    $soapRequest = Create-SOAPRequest -contractStrGUID $contractStrGUID -nomenclatureCode $nomenclatureCode
...
function Send-SOAPRequestWebClient {
    ...
    try {
        ...
        $webClient.Headers.Add("SOAPAction", "http://tempuri.org/GetMDInfo_By_ContractIDAndNomenclatureCode")
...
$totalCombinations = $nomenclatureCodes.Count * $contractStrGUIDs.Count
$current = 0

foreach ($nomenclatureCode in $nomenclatureCodes) {
    foreach ($contractStrGUID in $contractStrGUIDs) {
        ...
        Write-Host "NomenclatureCode: $nomenclatureCode" -ForegroundColor Cyan
        
        # Генерация имен файлов
        $requestFileName = "$contractStrGUID, $nomenclatureCode-Запрос из PS.XML"
        $responseFileName = "$contractStrGUID, $nomenclatureCode-Ответ из PS.XML"
        ...
        $soapResponse = Send-SOAPRequestWebClient -contractStrGUID $contractStrGUID -nomenclatureCode $nomenclatureCode
        
        # Если первый способ не сработал, пробуем второй
        if (-not $soapResponse) {
            Write-Host "Попытка отправки через Invoke-WebRequest..." -ForegroundColor Gray
            $soapResponse = Send-SOAPRequest -contractStrGUID $contractStrGUID -nomenclatureCode $nomenclatureCode
        }
        ...
        if (-not $soapResponse -and $webProxy) {
            try {
                ...
                $soapResponse = $webProxy.GetMDInfo_By_ContractIDAndNomenclatureCode($contractStrGUID, $nomenclatureCode)
...
Write-Host "Создано файлов запросов: $($nomenclatureCodes.Count * $contractStrGUIDs.Count)" -ForegroundColor Green
Write-Host "Создано файлов ответов: $($nomenclatureCodes.Count * $contractStrGUIDs.Count)" -ForegroundColor Green