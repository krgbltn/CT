...
6. После успешной передачи агент направляет диалог к `aiAgent` на `nextArticle` из своих настроек. Сценарий статьи спрашивает клиента, хочет ли он передать показания по другому ЛС; при согласии запрашивает номер ЛС и запускает `identification_account_number`.
