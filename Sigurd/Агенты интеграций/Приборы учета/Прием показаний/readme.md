...
Ищет лицевые счета по телефону вызовом `GetContractsInfo_By_Phone`. Для выбранного ЛС запрашивает приборы вызовом `GetMDInfo_By_ContractIDAndNomenclatureCode` для кодов номенклатур `1`, `2`, `4`, `21`.
...
3. Для текущего ЛС он передает в сценарий номера приборов, предыдущие показания и GUID первых доступных шкал.
