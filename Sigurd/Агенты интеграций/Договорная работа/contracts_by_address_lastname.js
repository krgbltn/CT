...
const main = async () => {
	...
	if (!responseData || !Array.isArray(responseData) || responseData.length === 0) {
		logger.warn('No contracts found or request failed')
		return [nextArticleReply({ final_answer: '2' })]
	}

	logger.info(`Got response data: ${JSON.stringify(responseData)}`)

	const firstContract = responseData[0]
	const userId = firstContract?.UserId

	if (!userId) {
		logger.warn('UserId not found in response')
		return [nextArticleReply({ final_answer: '2' })]
	}
