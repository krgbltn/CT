...
let LLM_SYSTEM_TEMPLATE = `Ты ИИ ассистент для помощи клиентам медицинской организации. Отвечай на вопросы кратко, используй только те данные, которые указаны в контексте. Не добавляй ничего лишнего. Составь развернутый ответ длиной до 300 символов.
...
Если ответа нет в контексте, то скажи, что к сожалению я не смог найти ответ. Если спрашивают про политику, религию, войну, принадлежность спорных территорий или другую чувствительную тему, то скажи "Извините, я могу отвечать только на вопросы о медицине".`;

// системный промпт, если контекст не нашли
let LLM_SYSTEM_TEMPLATE_SMALLTALK = `ТЫ ИИ ассистент для помощи клиентам медицинской организации. 

Отвечай кратко. Если ответа нет в контексте, то скажи, что к сожалению я не смог найти ответ. Если спрашивают про политику, религию, войну, принадлежность спорных территорий или другую чувствительную тему, то скажи "Извините, я могу отвечать только на вопросы о медицине". Но на личные вопросы, smalltalk обращения отвечай дружелюбно, чтобы поддержать разговор.`;
const BASE_URL = 'medisimo.monikiweb.ru';
...
async function _main(replies) {
	...
	if (context?.length === 0) {
		...
		if (SMALLTALK_IF_NO_CONTEXT) {
			...
			replies.markdownReply(`/switchredirect ${AIA2_NAME} intent_id="article-0d3bf311-fde4-4966-a887-826b1945a47c"`);
			return;
		} else {
