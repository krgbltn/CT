...
async function run(message) {
	...
	try {
		...
		if (data.role_BD && data.city) {
			...
			const cityRoles = CITY_ROLES[data.city]

			if (citySlotName && cityRoles) {
				const roleSlotValue = cityRoles[data.role_BD] || cityRoles["Другая роль"]
				newSlots[citySlotName] = roleSlotValue
				logger.info(`Role mapping: ${data.city} -> ${data.role_BD} -> ${roleSlotValue}`)
			} else {
