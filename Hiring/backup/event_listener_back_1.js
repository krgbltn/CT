...
async function extlogger(txt, note = null) {
	logger.info(`Incoming extlogger`);
	try {
		logger.info(
...
async function fillSlots(message) {
	logger.info(`Incoming fillSlots ${message}`);
	...
	logger.info(`After TGTag in fillSlots`);
	...
	logger.info(`Incoming switchredirect ${ragent}`);
	return [
...
if (message.message_type === 200) {
	logger.info(`Incoming message type ${message.message_type}`);
...
async function updateAmoCrmFields(message, lead_id) {
	const cityMapping = {
		...
		city_248: "Астана",
		city_244: "Видное",
		city_245: "Электросталь",
		city_246: "Ногинск",
		city_249: "Казань",
		city_250: "Наро-Фоминск",
		city_251: "Обнинск",
	}
