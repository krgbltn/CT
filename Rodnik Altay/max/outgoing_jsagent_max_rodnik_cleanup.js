...
const LAST_RATING_MID_PREFIX = "last_rating_mid:"
		...
		case MESSAGE_TYPES.send_message:
			switch (message.data.message_type) {
				...
				case MESSAGE_TYPES_SEND_MESSAGE.FINISH_DIALOG:
					...
					const chatId = getSlotValue(SLOTS.maxChatId)
					await agentStorage.globalStorage.del(chatId)

					if (HAS_SCORE && !skipScore) {
						const lastRatingMid = await agentStorage.globalStorage.get(`${LAST_RATING_MID_PREFIX}${chatId}`)
						if (lastRatingMid) {
							await agentStorage.globalStorage.del(`${SCORE_SENT_PREFIX}${chatId}:${lastRatingMid}`)
							logger.info(`Deleted stale score_sent key for chat ${chatId}, mid ${lastRatingMid}`)
						}

						message.data.content.text = createRatingMessage()
						message.data.content.text_type = TEXT_FORMAT.markdown
						const ratingResult = await handleSendMessage(message.data)
						const newRatingMid = ratingResult?.data?.messages?.[0]?.message_id
						if (newRatingMid) {
							await agentStorage.globalStorage.set(`${LAST_RATING_MID_PREFIX}${chatId}`, newRatingMid)
						}
					}
