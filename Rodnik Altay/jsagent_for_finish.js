const getSlotValue = (slotId) => {
	const slot = message.slot_context.filled_slots.find((s) => s.slot_id == slotId)
...
async function run(){
	...
	switch(event) {
		...
		case "DialogFinished":
			...
			const finishedDialogId = getSlotValue("ep_DialogId")
			if (finishedDialogId) {
				const prevDialogId = await agentStorage.omniUserStorage.get("LAST_FINISHED_DIALOG")
				if (prevDialogId && prevDialogId !== finishedDialogId) {
					await agentStorage.omniUserStorage.del(`POST_RATING_SENT:${prevDialogId}`)
				}
				await agentStorage.omniUserStorage.set("LAST_FINISHED_DIALOG", finishedDialogId)
				await agentStorage.omniUserStorage.set(`FINISH_DIALOG:${finishedDialogId}`, true)
			} else {
		...
		case "DialogScoreReceived":
			...
			const scoredDialogId = getSlotValue("ep_DialogId")
			const finishDialogFlag = scoredDialogId
				? await agentStorage.omniUserStorage.get(`FINISH_DIALOG:${scoredDialogId}`)
				: await agentStorage.omniUserStorage.get("FINISH_DIALOG")
			...
			if (finishDialogFlag) {
				...
				if (scoredDialogId) {
					const alreadySent = await agentStorage.omniUserStorage.get(`POST_RATING_SENT:${scoredDialogId}`)
					if (alreadySent) {
						logger.info(`Post-rating scenario already sent for dialog ${scoredDialogId}. Skip duplicate.`)
						return []
					}
					await agentStorage.omniUserStorage.set(`POST_RATING_SENT:${scoredDialogId}`, true)
					await agentStorage.omniUserStorage.del(`FINISH_DIALOG:${scoredDialogId}`)
				} else {
