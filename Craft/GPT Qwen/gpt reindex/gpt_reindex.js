...
async function main() {
	...
	try {
		response = await axios.post(URL_REINDEX, {
			autodeploy: true, chunk_size: 2000, customer_id: ['umka', 'docs', 'pony'], format: {
				strategy: "last_headers", n_headers: 3, delimiter: " >>> ", add_title: false
			}, do_index_messages: false, text_analyzer: "russian",
		})
		...
		//throw e;
		return ([agentApi.makeTextReply(`Error: ${JSON.stringify(e)}`),])
	}

	return ([agentApi.makeTextReply(`Created a new index for context search: ${JSON.stringify(response.data)}`),])
}
