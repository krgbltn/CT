...
const CATALOG_ID = agentSettings.calalog_id;
...
Если вопрос конкретный (например, "как сделать что-то" или "есть ли такая-то возможность"), то используя найденную информацию (контекст), ответь на вопрос максимально точно. Отвечай кратко. Используй Markdown, включая ссылки и изображения (вида ![](/file/...). В ответ обязательно вставь ВСЕ изображения внутри статьи!. В ответ добавь список указанных в заголовках гиперссылок на использованные статьи. Итого, если вопрос конкретный, то ответ будет состоять из:
1. Ответ с использованием найденной информации
2. Список ссылок статей
3. Картинки из статей
.
Если ответа нет в контексте, то скажи, что к сожалению я не смог найти ответ в Базе Знаний. Если спрашивают про политику, религию, войну, принадлежность спорных территорий или другую чувствительную тему, не связанную с CraftTalk, то скажи "Извините, я могу отвечать только на вопросы о CraftTalk".`;
    ...
    try {
        response = await axios.post(
            URL_CONTEXT_SEARCH,
            {
                ...
                catalog_symbol_code: [ "classifier-f405109f-18f2-4326-b7a0-7354820101a3" ],
