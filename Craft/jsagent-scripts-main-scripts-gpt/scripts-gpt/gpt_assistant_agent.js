...
const URL_MEDIATOR = agentSettings.url_mediator_service;
...
async function gptReplicas(detailedAnswer=false) {
    ...
    const clientText = message.messages?.filter(x => x.side === "client")?.[0]?.text;

    const dialogHistory = await getDialog(message.dialog_id);
    if (dialogHistory && dialogHistory.length > 0) {
        for (let i = dialogHistory.length - 1; i >= 0; i--) {
            if (dialogHistory[i].role === "user") {
                if (dialogHistory[i].message === clientText) {
        ...
        logger.info(`No dialog history for dialog_id: ${message.dialog_id}`);
    }
...
function findLastSwitchRedirectIndex(history) {
    if (!Array.isArray(history)) return -1;
    for (let i = history.length - 1; i >= 0; i--) {
        const userText = history[i].msg?.message?.text || '';
        const replyText = history[i].reply?.message?.text || '';
        if (userText.includes('/switchredirect') || replyText.includes('/switchredirect')) {
            return i;
        }
    }
    return -1;
...
async function getDialog(dialog_id) {
    if (!dialog_id) return [];
    try {
        const response = await axios.get(`${URL_MEDIATOR}?dialog_id=${dialog_id}`, {
            timeout: 3000
        });
        const data = response.data;
        if (!data || !Array.isArray(data)) return [];

        const switchIdx = findLastSwitchRedirectIndex(data);
        let history;
        if (switchIdx !== -1) {
            let lastUserMsgBeforeSwitch = null;
            for (let i = switchIdx - 1; i >= 0; i--) {
                const item = data[i];
                if (item.msg && item.msg.message_type === MESSAGE_TYPES.MESSAGE) {
                    const text = item.msg?.message?.text || '';
                    if (text && !text.includes('/switchredirect')) {
                        lastUserMsgBeforeSwitch = item;
                        break;
                    }
                }
            }
            history = data.slice(switchIdx + 1);
            if (lastUserMsgBeforeSwitch) {
                history.unshift(lastUserMsgBeforeSwitch);
            }
        } else {
            history = data;
        }
        ...
        return history.reduce((acc, item) => {
            ...
            if (type === MESSAGE_TYPES.MESSAGE && text) {
                acc.push({ role: isUser ? "user" : "assistant", message: text });
            }
            return acc;
        }, []);
    } catch (e) {
        logger.error(`Error getDialog: ${e}.`);
        return [];
    }
}
