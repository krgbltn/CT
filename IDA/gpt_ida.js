...
1. Уточняющие вопросы (если уместно)
2. Список ссылок статей
3. Картинки из статей
4. Ответ с использованием найденной информации
.
...
let LLM_SYSTEM_TEMPLATE_SMALLTALK = agentSettings.smalltalk_prompt /*`ТЫ ИИ ассистент для помощи клиентам компании ИДА. ИДА - это AI-платформа управления знаниями и текстовыми коммуникациями для поддержки, маркетинга и продаж с использованием технологий искусственного интеллекта. Платформа создана для оказания услуг контакт-центра в текстовом режиме, работающий в web-чатах, мессенджерах, социальных сетях, e-mail. В Базе Знаний содержится информация о настройке и использовании платформы, чат-ботов, баз знаний, каналов, и т.п.
...
async function getContext(question, replies) {
	...
	try {
		const payload = {
			...
			catalog_symbol_code: CATALOG_ID,
			output_format: "json-vikhr"
		};

		logger.info(`>>> POST ${URL_CONTEXT_SEARCH}`);
		logger.info(`>>> Payload: ${JSON.stringify(payload)}`);
		logger.info(`>>> catalog_symbol_code type: ${typeof CATALOG_ID}, isArray: ${Array.isArray(CATALOG_ID)}`);
		response = await axios.post(
			URL_CONTEXT_SEARCH,
			{
				text: question,
...
//                record_type: RECORD_TYPE,
				catalog_symbol_code: CATALOG_ID, //["classifier-8f20dd02-5cbb-4739-8d18-dbcd3d96ae70"],
				output_format: "json-vikhr"
			}
		);
...
function _debugAxiosError(error, replies) {
	if (error.response) {
		...
		if (error.response?.data?.detail) {
			logger.info(`422 DETAIL: ${JSON.stringify(error.response.data.detail)}`);
			// Выводим каждый элемент массива отдельно для читаемости
			error.response.data.detail.forEach((err, i) => {
				logger.info(`  [${i}] loc:${err.loc?.join('.')} | msg:${err.msg} | type:${err.type}`);
			});
		}
