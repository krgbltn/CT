...
async function tryOverrideDialogRouting(responseVariables, overrideDialogRoutingSettings, overrideDialogRoutingVariable, redirectToOperatorAction) {
	if (responseVariables[overrideDialogRoutingVariable]) {
		let overrideDialogRoutingValue = responseVariables[overrideDialogRoutingVariable]

		if (overrideDialogRoutingSettings[overrideDialogRoutingValue]) {
			let routing = overrideDialogRoutingSettings[overrideDialogRoutingValue]
			await agentStorage.agentStorage.del(message.user.omni_user_id)
			resolve([
				agentApi.makeTextReply(
					'Перевожу вас на оператора, он постарается помочь.',
					undefined,
					undefined,
					{isOperator: "true", routing_queue: routing }),
				agentApi.makeTextReply(
					`/switchredirect ${redirectToOperatorAction[message.channel.channel_id] ?? redirectToOperatorAction.default_routing}`
				)])
...
async function runAction(action, variables, overrideDialogRoutingSettings, overrideDialogRoutingVariable, dialogId, omniUserId) {
	...
	if (action.transfer) {
		...
		await tryOverrideDialogRouting(variables, overrideDialogRoutingSettings, overrideDialogRoutingVariable, agentConfig.RedirectToOperatorAction)
